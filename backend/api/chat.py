...
СТИЛЬ РОБОТИ:
- Відповідай мовою клієнта (UA/PL/EN/RU)
- Якщо запит російською — відповідай РОСІЙСЬКОЮ мовою
...
- DN (Діаметральний Номінальний) = ВНУТРІШНІЙ діаметр шланга в мм
  По-російськи: ДН (Диаметр Номинальный) = внутрішній діаметр. "ДН65" = DN65 = 65мм
  По-польськи: DN (Średnica Nominalna)
...
def _extract_tech_params(q: str) -> dict:
    ...
    # Normalize дн/ДН/ДН (Russian ДН = DN) 
    q = re.sub(r'\bдн\s*(\d+)\b', lambda m: f'DN{m.group(1)}', q, flags=re.I)
    q = re.sub(r'\bдн\b', 'DN', q, flags=re.I)
    # Russian бар → bar
    q = re.sub(r'\bбар\b', 'bar', q, flags=re.I)
    # Russian атм ≈ 1 bar
    q = re.sub(r'(\d+)\s*атм\b', lambda m: f'{m.group(1)} bar', q, flags=re.I)
    ...
    try:
        ...
        async with engine.connect() as conn:
            res = await conn.execute(
                text(f"SELECT id FROM products WHERE embedding IS NOT NULL ORDER BY embedding <=> '{str(emb)}'::vector LIMIT {limit}")
            )
